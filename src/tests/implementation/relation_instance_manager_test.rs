...
use crate::builder::{
    EntityInstanceBuilder, EntityTypeBuilder, RelationInstanceBuilder, RelationTypeBuilder,
};
...
fn test_relation_instance_manager() {
    ...
    let entity_type = EntityTypeBuilder::new(outbound_type.clone())
        .string_property(property_name.clone())
        .build();
    entity_type_manager.register(entity_type.clone());
    let entity_type = EntityTypeBuilder::new(inbound_type.clone())
        .string_property(property_name.clone())
        .build();
    entity_type_manager.register(entity_type.clone());
    ...
    let relation_instance =
        RelationInstanceBuilder::new(Uuid::new_v4(), type_name.clone(), inbound_id)
            .property(property_name.clone(), property_value.clone())
            .get();
    let result = relation_instance_manager.create_from_instance(relation_instance.clone());
    assert!(result.is_err());
    ...
    let relation_instance =
        RelationInstanceBuilder::new(outbound_id, type_name.clone(), Uuid::new_v4())
            .property(property_name.clone(), property_value.clone())
            .get();
    let result = relation_instance_manager.create_from_instance(relation_instance.clone());
    assert!(result.is_err());
    ...
    let relation_instance =
        RelationInstanceBuilder::new(outbound_id, type_name.clone(), inbound_id.clone())
            .property(property_name.clone(), property_value.clone())
            .get();
    let result = relation_instance_manager.create_from_instance(relation_instance.clone());
    assert!(result.is_ok());
    ...
    let wrong_outbound_id_edge_key = EdgeKey::new(
        Uuid::new_v4(),
        Type::new(type_name.clone()).unwrap(),
        inbound_id,
    );
    let wrong_inbound_id_edge_key = EdgeKey::new(
        outbound_id,
        Type::new(type_name.clone()).unwrap(),
        Uuid::new_v4(),
    );
    ...
    assert!(relation_instance_manager
        .get(wrong_outbound_id_edge_key.clone())
        .is_none());
    assert!(relation_instance_manager
        .get(wrong_inbound_id_edge_key.clone())
        .is_none());
...
fn test_relation_instance_manager_import_export() {
    ...
    path.push(format!(
        "{}-{}-{}.json",
        outbound_id.to_string().as_str(),
        type_name.clone(),
        inbound_id.to_string().as_str()
    ));
    let path = path.into_os_string().into_string().unwrap();

    let relation_type = RelationTypeBuilder::new(outbound_type, type_name.clone(), inbound_type)
        .string_property(property_name.clone())
        .build();
    relation_type_manager.register(relation_type.clone());
