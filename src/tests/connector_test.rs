...
const PARSE_INT_CONNECTOR_OPERATION: fn(Value) -> Value =
    |v| json!(v.clone().as_str().unwrap().parse::<i64>().unwrap());
...
fn connector_destruction_test() {
    ...
    assert_eq!(
        e3_default_string.clone(),
        e3.as_string(e3_property_name.clone()).unwrap()
    );
    let s = r_string();
    e1.set(e1_property_name.clone(), json!(s.clone()));
    assert_eq!(s, e2.as_string(e2_property_name.clone()).unwrap());
    assert_eq!(
        e3_default_string.clone().to_string(),
        e3.as_string(e3_property_name.clone()).unwrap()
    );
}
...
pub(crate) fn create_default_connector<S: Into<String>>(
    ...
    create_relation_instance_with_properties(
        outbound_entity,
        Connector::type_name(DEFAULT_CONNECTOR_TYPE_NAME, "bit_1", "bit_1"),
        inbound_entity,
