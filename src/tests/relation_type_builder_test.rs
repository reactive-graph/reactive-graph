...
fn relation_type_builder_test() {
    ...
    let outbound_type = EntityTypeType::new_from_type(&outbound_type_namespace, &outbound_type_name);
    let inbound_type_namespace = r_string();
    let inbound_type_name = r_string();
    let inbound_type = EntityTypeType::new_from_type(&inbound_type_namespace, &inbound_type_name);
