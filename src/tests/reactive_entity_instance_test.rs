...
use crate::{ReactiveEntityInstance, ReactivePropertyInstance, EntityInstance};
use indradb::{VertexProperties, Vertex, Type, NamedProperty};
use serde_json::json;
use std::collections::HashMap;
use std::str::FromStr;
use uuid::Uuid;
use crate::tests::utils::{r_string, r_json_string, create_random_entity_instance};
use std::sync::Arc;
...
fn reactive_entity_instance_test() {
    ...
    properties.insert(property_name.clone(), ReactivePropertyInstance::new(
        Uuid::new_v4(),
        property_name.clone(),
