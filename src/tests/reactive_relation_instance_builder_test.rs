...
fn reactive_relation_instance_builder_test() {
    ...
    let relation_instance = ReactiveRelationInstanceBuilder::new_from_type(&relation_type_namespace, outbound, &relation_type_name, inbound)
    ...
    assert_eq!(relation_type_ty, relation_instance.ty);
...
fn reactive_relation_instance_builder_set_property_defaults_test() {
    ...
    let relation_type = RelationTypeBuilder::new(entity_type_1_ty.clone(), relation_type_ty.clone(), entity_type_2_ty.clone())
        ...
        .build();
    let relation_instance = ReactiveRelationInstanceBuilder::new(outbound, relation_type_ty.clone(), inbound)
