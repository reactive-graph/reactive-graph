...
fn reactive_relation_instance_builder_test() {
    ...
    let entity_type = EntityTypeBuilder::new(namespace.clone(), entity_type_name.clone()).build();
    let outbound_id = Uuid::new_v4();
    let inbound_id = Uuid::new_v4();
    let outbound = ReactiveEntityInstanceBuilder::from(entity_type.clone()).id(outbound_id).build();
    let inbound = ReactiveEntityInstanceBuilder::from(entity_type.clone()).id(inbound_id).build();
