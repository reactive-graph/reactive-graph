...
#[wrapper]
pub struct ReactiveRelationInstances(
    std::sync::RwLock<std::collections::HashMap<EdgeKey, std::sync::Arc<ReactiveRelationInstance>>>,
);
...
impl ReactiveRelationInstanceManager for ReactiveRelationInstanceManagerImpl {
    ...
    fn get_by_outbound_entity(
        ...
        self.relation_edge_manager
            ...
            .filter_map(|edge| {
                reader
                    .get(&edge.key.clone())
                    .and_then(|relation_instance| Some(relation_instance.clone()))
            })
            .collect()
    }
    ...
    fn create(
        ...
        if result.is_err() {
            return Err(
                ReactiveRelationInstanceCreationError::RelationInstanceCreationError(
                    result.err().unwrap(),
                ),
            );
        }
    ...
    fn create_reactive_instance(
        ...
        if outbound.is_none() {
            return Err(
                ReactiveRelationInstanceCreationError::MissingOutboundEntityInstance(
        ...
        if outbound.is_none() {
            return Err(
                ReactiveRelationInstanceCreationError::MissingInboundEntityInstance(
    ...
    fn register_reactive_instance(
        ...
        if edge_key.is_some() {
            ...
            let _result = self
                .relation_instance_manager
                .create_from_instance(reactive_relation_instance.clone().into());
    ...
    fn commit(&self, edge_key: EdgeKey) {
        ...
        if reactive_relation_instance.is_some() {
            self.relation_instance_manager
                .commit(reactive_relation_instance.unwrap().into());
        }
    }
