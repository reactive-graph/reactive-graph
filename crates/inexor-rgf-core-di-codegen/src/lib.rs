...
pub fn wrapper(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let type_to_wrap = if let Fields::Unnamed(fields) = &wrapper.fields {
        ...
        if field.is_none() {
            return TokenStream::from(Error::new(wrapper.span(), "Struct annotated #[wrapper] must have exactly one field").to_compile_error());
        }

        field.unwrap().ty.clone()
    } else {
        return TokenStream::from(Error::new(wrapper.span(), "Only tuple like struct supported for #[wrapper]").to_compile_error());
    };
...
fn remove_attrs(item: TokenStream) -> TokenStream {
    ...
    let item = match item {
        Item::Fn(mut fn_) => {
            ...
            fn_.sig.inputs.iter_mut().for_each(|arg| {
                if let FnArg::Typed(path) = arg {
