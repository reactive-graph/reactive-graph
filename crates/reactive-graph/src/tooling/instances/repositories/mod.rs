...
pub fn handle_repository(instance_dir: &PathBuf, args: RepositoriesArgs) {
    match args.commands {
        RepositoriesCommands::Init(args) => {
            init_repository(instance_dir, args);
        }
        RepositoriesCommands::Remove(args) => {
            remove_repository(instance_dir, args);
        }
    }
}
