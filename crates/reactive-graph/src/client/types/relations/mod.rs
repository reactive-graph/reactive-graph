...
pub(crate) async fn relation_types(client: &Arc<ReactiveGraphClient>, relation_type_args: RelationTypesArgs) -> CommandResult {
    ...
    match command {
        RelationTypesCommands::List => match client.types().relation_types().get_all_relation_types().await {
        ...
        RelationTypesCommands::Get(args) => match client.types().relation_types().get_relation_type_by_type(args.clone()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(args.not_found()),
            Err(e) => Err(e.into()),
        },
        RelationTypesCommands::ListProperties(args) => match client.types().relation_types().get_relation_type_by_type(args.clone()).await {
        ...
        RelationTypesCommands::ListExtensions(args) => match client.types().relation_types().get_relation_type_by_type(args.clone()).await {
            Ok(Some(relation_type)) => {
        ...
        RelationTypesCommands::ListComponents(args) => match client.types().relation_types().get_relation_type_components(args.clone()).await {
            Ok(Some(components)) => {
        ...
        RelationTypesCommands::Create(args) => match client.types().relation_types().create_relation_type_with_variables((&args).into()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(NotCreated("Relation type wasn't created".to_string())),
            Err(e) => Err(e.into()),
        },
        RelationTypesCommands::Delete(args) => match client.types().relation_types().delete_relation_type_with_variables((&args).into()).await {
        ...
        RelationTypesCommands::AddProperty(args) => match client.types().relation_types().add_property_with_variables((&args).into()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(NotCreated("Property wasn't created".to_string())),
            Err(e) => Err(e.into()),
        },
        RelationTypesCommands::RemoveProperty(args) => match client.types().relation_types().remove_property_with_variables((&args).into()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(NotFound(format!("Relation type {}__{} not found", args.ty.namespace, args.ty.name))),
            Err(e) => Err(e.into()),
        },
        RelationTypesCommands::AddExtension(args) => match client.types().relation_types().add_extension_with_variables((&args).into()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(NotCreated("Extension wasn't created".to_string())),
            Err(e) => Err(e.into()),
        },
        RelationTypesCommands::RemoveExtension(args) => match client.types().relation_types().remove_extension_with_variables((&args).into()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(args.relation_ty.not_found()),
            Err(e) => Err(e.into()),
        },
        RelationTypesCommands::AddComponent(args) => match client.types().relation_types().add_component((&args).into()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(NotCreated("Component wasn't added to relation type".to_string())),
            Err(e) => Err(e.into()),
        },
        RelationTypesCommands::RemoveComponent(args) => match client.types().relation_types().remove_component((&args).into()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(args.ty.not_found()),
            Err(e) => Err(e.into()),
        },
        RelationTypesCommands::UpdateDescription(args) => match client.types().relation_types().update_description_with_variables((&args).into()).await {
            Ok(Some(relation_type)) => output_format_wrapper.single(relation_type),
            Ok(None) => Err(args.ty.not_found()),
            Err(e) => Err(e.into()),
        },
