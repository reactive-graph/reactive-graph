...
pub(crate) async fn relation_instances(client: &Arc<InexorRgfClient>, relation_instances_args: RelationInstancesArgs) -> CommandResult {
    ...
    match command {
        ...
        //     Err(e) => Err(e.into()),
        // },
        // RelationInstancesCommands::ListProperties(args) => match client.instances().relation_instances().get_relation_instance_by_id(args.clone()).await {
        ...
        //     Err(e) => Err(e.into()),
        // },
        // RelationInstancesCommands::GetProperty(args) => match client.instances().relation_instances().get_relation_instance_by_id(args.id).await {
        ...
        //     .await
        // {
        //     Ok(Some(relation_instance)) => Ok(relation_instance.get(args.name.clone()).ok_or(args.property_not_found())?.into()),
        //     Ok(None) => Err(args.id_not_found()),
        //     Err(e) => Err(e.into()),
        // },
        ...
        //     match client.instances().relation_instances().remove_property(args.id, args.property_name.clone()).await {
        //         Ok(Some(relation_instance)) => output_format_wrapper.single(relation_instance),
        //         Ok(None) => Err(args.id_not_found()),
        //         Err(e) => Err(e.into()),
        //     }
        // }
        // RelationInstancesCommands::ListComponents(args) => match client.instances().relation_instances().get_relation_instance_by_id(args.clone()).await {
        //     Ok(Some(relation_instance)) => {
        ...
        //     match client.instances().relation_instances().add_component(args.id, component_ty).await {
        //         Ok(Some(relation_instance)) => output_format_wrapper.single(relation_instance),
        //         Ok(None) => Err(args.id_not_found()),
        //         Err(e) => Err(e.into()),
        //     }
        ...
        //     let component_ty: ComponentTypeId = args.component_ty.clone().into();
        //     match client.instances().relation_instances().remove_component(args.id, component_ty).await {
        //         Ok(Some(relation_instance)) => output_format_wrapper.single(relation_instance),
        //         Ok(None) => Err(args.id_not_found()),
        //         Err(e) => Err(e.into()),
        //     }
        ...
        //     Err(e) => Err(e.into()),
        // },
        // RelationInstancesCommands::Delete(args) => match client.instances().relation_instances().delete_relation_instance(args.id).await {
        //     Ok(Some(true)) => Ok(format!("Relation instance {} deleted", args.id).into()),
        //     Ok(Some(false)) => Ok(format!("Relation instance {} not deleted", args.id).into()),
        //     Ok(None) => Err(args.not_found()),
        //     Err(e) => Err(e.into()),
        // },
    }
