...
use crate::{AddExtensionError, Namespaces};
...
impl Component {
    pub fn new<T: Into<ComponentTypeId>, D: Into<String>, P: Into<PropertyTypes>, E: Into<Extensions>>(ty: T, description: D, properties: P, extensions: E) -> Component {
        Component {
            ty: ty.into(),
    ...
    pub fn new_from_type<S: Into<String>, T: Into<String>, D: Into<String>, P: Into<PropertyTypes>, E: Into<Extensions>>(namespace: S, type_name: T, description: D, properties: P, extensions: E) -> Component {
        Component {
...
impl Components {
    ...
    //     self.0.insert(component.ty.clone(), component);
    // }
    //
    // pub fn to_vec(&self) -> Vec<Component> {
    //     let mut components: Vec<Component> = self.0.iter().map(|component| component.value().clone()).collect();
    //     components.sort();
    //     components
    // }
    ...
    // pub fn contains_ty<C: Into<ComponentTypeId>>(&self, ty: C) -> bool {
    //     let ty = ty.into();
    //     self.0.iter().any(|component| component.ty == ty)
    // }
    ...
    pub fn get<T: Into<ComponentTypeId>>(&self, ty: T) -> Option<Component> {
        let ty = ty.into();
        self.0.iter()
            .find(|component| component.ty == ty)
            .map(|component| component.clone())
    }

    // pub fn get_mut<'a, T: Into<ComponentTypeId>>(&mut self, ty: T) -> Option<&Component> {
    //     let ty = ty.into();
    //     self.0.iter()
    //         .find(|component| component.ty == ty)
    ...
    pub fn get_by_types<C: Into<ComponentTypeIds>>(&self, tys: C) -> Components {
        ...
        // self.0.iter()
        //     .filter(|component| tys.contains(&component.ty))
        //     .map(|component| component.clone())
        //     .collect()
    }
    ...
    fn filter_by<P>(&self, predicate: P) -> Components
        where P: FnMut(&RefMulti<ComponentTypeId, Component>) -> bool
    {
        self.0.iter()
            .filter(predicate)
            .map(|component| component.clone())
...
impl PartialEq for Components {
    fn eq(&self, other: &Self) -> bool {
        self.0.iter().all(|self_component| other.contains_key(&self_component.ty))
            && other.iter().all(|other_component| self.contains_key(&other_component.ty))
    }
}
...
mod component_type_tests {
    ...
    #[test]
    fn build_component() {
        let component = Component::builder().ty(ComponentTypeId::new_from_type("x", "y")).description("d").properties(Vec::new()).extensions(Vec::new()).build();
