...
impl FlowType {
    ...
    pub fn new_from_type<N: Into<String>, T: Into<String>, D: Into<String>, EI: Into<EntityInstances>, RI: Into<RelationInstances>, V: Into<PropertyTypes>, E: Into<Extensions>>(
        namespace: N,
        type_name: T,
        description: D,
        wrapper_entity_instance: EntityInstance,
...
impl NamespacedTypeEntityInstanceContainer<FlowTypeId, FlowTypeDoesNotExistError, FlowTypeAddEntityInstanceError, FlowTypeUpdateEntityInstanceError, FlowTypeRemoveEntityInstanceError> for FlowTypes {
...
impl NamespacedTypeRelationInstanceContainer<FlowTypeId, FlowTypeDoesNotExistError, FlowTypeAddRelationInstanceError, FlowTypeUpdateRelationInstanceError, FlowTypeRemoveRelationInstanceError> for FlowTypes {
...
impl NamespacedTypeVariablesContainer<FlowTypeId, FlowTypeAddVariableError, FlowTypeUpdateVariableError, FlowTypeRemoveVariableError, FlowTypeMergeVariablesError> for FlowTypes {
    fn add_variable<P: Into<Variable>>(&self, flow_ty: &FlowTypeId, variable: P) -> Result<Variable, FlowTypeAddVariableError> {
        ...
        flow_type
            ...
            .map_err(|e| FlowTypeAddVariableError::AddVariableError(e))
    }

    fn update_variable<N: Into<String>, V: Into<Variable>>(&self, flow_ty: &FlowTypeId, variable_name: N, variable: V) -> Result<Variable, FlowTypeUpdateVariableError> {
        let Some(flow_type) = self.0.get_mut(flow_ty) else {
...
impl NamespacedTypeExtensionContainer<FlowTypeId, FlowTypeAddExtensionError, FlowTypeUpdateExtensionError, FlowTypeRemoveExtensionError, FlowTypeMergeExtensionsError> for FlowTypes {
    fn add_extension<E: Into<Extension>>(&self, flow_ty: &FlowTypeId, extension: E) -> Result<ExtensionTypeId, FlowTypeAddExtensionError> {
        ...
        flow_type
            ...
            .map_err(|e| FlowTypeAddExtensionError::AddExtensionError(e))
    }

    fn update_extension<T: Into<ExtensionTypeId>, E: Into<Extension>>(&self, flow_ty: &FlowTypeId, extension_ty: T, extension: E) -> Result<Extension, FlowTypeUpdateExtensionError> {
        let Some(flow_type) = self.0.get_mut(flow_ty) else {
...
mod tests {
    ...
    fn create_flow_type_test() {
        ...
        assert_eq!(relation_instance_1.outbound_id, flow_type.relation_instances.get(&relation_instance_1.id()).unwrap().outbound_id);
        assert_eq!(relation_instance_2.outbound_id, flow_type.relation_instances.get(&relation_instance_2.id()).unwrap().outbound_id);
        assert_eq!(relation_instance_3.outbound_id, flow_type.relation_instances.get(&relation_instance_3.id()).unwrap().outbound_id);
        ...
        flow_type.add_entity_instance(entity_instance_4.clone()).expect("Failed to add entity instance 4");
        assert_eq!(4, flow_type.entity_instances().len());
        assert!(flow_type.has_entity_instance(entity_instance_4.id));
        flow_type.remove_entity_instance(entity_instance_4.id).expect("Failed to remove entity instance 4");
        assert_eq!(3, flow_type.entity_instances().len());
        assert!(!flow_type.has_entity_instance(entity_instance_4.id));
