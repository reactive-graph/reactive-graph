...
impl EntityInstance {
    ...
    pub fn new_without_properties<T: Into<EntityTypeId>>(ty: T, id: Uuid) -> EntityInstance { // , P: Into<PropertyInstances>
        EntityInstance {
            ty: ty.into(),
            id,
            description: String::new(),
...
impl EntityInstances {
    ...
    pub fn get_type_ids(&self) -> EntityTypeIds {
        self.iter()
            .map(|entity_instance| entity_instance.ty.clone())
            .collect()
    }
}
...
impl From<Vec<EntityInstance>> for EntityInstances {
    fn from(entity_instances: Vec<EntityInstance>) -> Self {
        Self(entity_instances.into_iter().map(|entity_instance| (entity_instance.id, entity_instance)).collect())
    }
}
  ...
  "extensions": [
    ...
    },
    {
      ...
      "extension": "other_extension_value"
        ...
        assert!(entity_instance.extensions.has_own_extension(&ExtensionTypeId::new_from_type("ext_namespace", "ext_name")));
        assert_eq!(json!("extension_value"), entity_instance.extensions.get_own_extension(&ExtensionTypeId::new_from_type("ext_namespace", "ext_name")).unwrap().extension);
        assert!(entity_instance.extensions.has_own_extension(&ExtensionTypeId::new_from_type("other_ext_namespace", "other_ext_name")));
        assert_eq!(json!("other_extension_value"), entity_instance.extensions.get_own_extension(&ExtensionTypeId::new_from_type("other_ext_namespace", "other_ext_name")).unwrap().extension);
    }
