...
#[wrapper]
pub struct ReactiveEntities(DashMap<Uuid, ReactiveEntity>);
...
#[provides]
fn create_reactive_entity_instances_storage() -> ReactiveEntities {
    ReactiveEntities(DashMap::new())
}
...
pub struct ReactiveEntityManagerImpl {
    ...
    reactive_entity_instances: ReactiveEntities,
...
impl SystemEventSubscriber for ReactiveEntityManagerImpl {
...
impl ReactiveEntityManager for ReactiveEntityManagerImpl {
    fn has(&self, id: Uuid) -> bool {
        self.reactive_entity_instances.0.contains_key(&id)
    }
    ...
    fn create(&self, ty: &EntityTypeId, properties: PropertyInstances) -> Result<ReactiveEntity, ReactiveEntityCreationError> {
        ...
        // if result.is_err() {
        //     return Err(ReactiveEntityCreationError::EntityInstanceCreationError(result.err().unwrap()));
        // }
    ...
    fn register_reactive_instance(&self, reactive_entity: ReactiveEntity) -> Result<ReactiveEntity, ReactiveEntityRegistrationError> {
        ...
        self.event_manager.emit_event(SystemEvent::EntityInstanceCreated(reactive_entity.id));
        Ok(reactive_entity)
        //
        // match self
    ...
    //                 Err(error) => Err(ReactiveEntityImportError::ReactiveEntityCreation(error)),
    //             },
    //             None => Err(ReactiveEntityImportError::MissingEntityInstance(uuid)),
    //         },
    //         Err(error) => Err(ReactiveEntityImportError::EntityInstanceImport(error)),
    //     }
    // }
    //
