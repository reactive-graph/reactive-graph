...
impl MutationFlowTypes {
    ...
    async fn add_entity_instance(
        ...
        if flow_type_manager.has(&ty) {
            return Err(Error::new(format!("Flow type {ty} already exists")));
        }
        if flow_type_manager.add_entity_instance(&ty, entity_instance.into()).is_err() {
            return Err(Error::new(format!("Failed to add entity instance to flow type {ty}")));
        }
        Ok(true)
    }
    ...
    async fn update_extension(
        ...
        let flow_type_manager = context.data::<Arc<dyn FlowTypeManager>>()?;
        let flow_ty: FlowTypeId = ty.into();
