...
pub fn get_relation_mutation_types(mut schema: SchemaBuilder, context: &SchemaBuilderContext) -> SchemaBuilder {
    for relation_type in context.relation_type_manager.get_all() {
        schema = schema.register(get_relation_mutation_type(relation_type));
    }
    schema
}
...
pub fn get_relation_delete_field() -> Field {
    Field::new("delete", TypeRef::named_nn_list_nn(TypeRef::ID), move |ctx| {
        FieldFuture::new(async move {
            ...
            let mut ids = Vec::new();
            for relation_instance in ctx.parent_value.try_downcast_ref::<Vec<Arc<ReactiveRelationInstance>>>()? {
                trace!("Deleting relation instance {}", relation_instance);
                let id = relation_instance.ty.to_string();
                let key = relation_instance.get_key();
                relation_instance_manager.delete(&key);
                ids.push(id);
            }
            Ok(Some(FieldValue::list(ids.iter().map(|id| FieldValue::value(ID(id.clone()))))))
        })
    })
}
