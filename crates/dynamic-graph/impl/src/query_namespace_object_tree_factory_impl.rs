...
use reactive_graph_dynamic_graph_api::ROOT_OBJECT_NAME_QUERY;
...
#[derive(Component)]
pub struct QueryNamespaceObjectTreeFactoryImpl {
...
impl QueryNamespaceObjectTreeFactoryImpl {}

#[async_trait]
#[component_alias]
impl QueryNamespaceObjectTreeFactory for QueryNamespaceObjectTreeFactoryImpl {
    fn get_namespace_objects(&self) -> Vec<Object> {
        ...
        for namespace in namespaces {
            ...
            // Create fields
            for field in self.component_query_field_factory.create_query_fields(&namespace) {
                object = object.field(field);
            }
            for field in self.entity_query_field_factory.create_query_fields(&namespace) {
                object = object.field(field);
            }
            for field in self.relation_query_field_factory.create_query_fields(&namespace) {
                object = object.field(field);
            }
            for field in self.flow_query_field_factory.create_query_fields(&namespace) {
                object = object.field(field);
            }
            object_tree.insert(namespace.clone(), object);
            // Create field in parent object or root object if there is no parent object
            let field = namespace_field(namespace.clone(), Query);
            ...
            match namespace.parent() {
                None => {
                    ...
                    trace!("Create field {namespace_field_name} for Namespace({namespace}) in root object {ROOT_OBJECT_NAME_QUERY}");
                }
                Some(parent_namespace) => {
                    ...
                    match object_tree.remove(&parent_namespace) {
                        ...
                        Some(parent_object) => {
                            ...
                            trace!("Create field {namespace_field_name} for Namespace({namespace}) in parent Namespace({parent_namespace})");
                        }
                    }
                }
            }
    ...
    fn create_namespace_object(&self, namespace: &Namespace) -> Object {
        let object_type_name = namespace_type_name(namespace, Query);
        trace!("Create namespace {Query} object {object_type_name} for Namespace({namespace})");
        let object = Object::new(object_type_name)
            .description(format!(
                "Queries for components, entities and relations on the namespace {}",
    ...
    fn create_root_object(&self) -> Object {
        trace!("Create root object {ROOT_OBJECT_NAME_QUERY}");
        Object::new(ROOT_OBJECT_NAME_QUERY)
            .description("Queries")
...
#[async_trait]
impl Lifecycle for QueryNamespaceObjectTreeFactoryImpl {}
