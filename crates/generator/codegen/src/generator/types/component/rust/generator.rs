...
use crate::component::rust::generate_const_type_definition;
use crate::component::rust::generate_impl_trait_property_instance_getter_and_setter_method_signatures;
...
impl TypeGenerator<Rust> for Component {
    fn generate_type(&self, config: &CodeGenerationConfig, resolver: &TypeResolver) -> Result<CodeGenerationResult, CodeGenerationError> {
        ...
        println!("Generating source code for component {} to {path:?}", self.namespace());
        ...
        let impl_trait_property_instance_getter_and_setter = generate_impl_trait_property_instance_getter_and_setter_method_signatures(self, resolver);
...
mod tests {
    ...
    use crate::TypeGeneratorForTarget;
    use crate::generator::types::TypeGenerator;
    use crate::targets::rust::Rust;
    use convert_case::Case::Snake;
    use convert_case::Casing;
    ...
    #[test]
    pub fn test_generate_component_rust_trait() {
        let component = Component::random_type().unwrap();
        let resolver = TypeResolver::new();

        let config = CodeGenerationConfig::with_temp_dir();
        println!("fully qualified namespace: {}", component.namespace());
        let target_path = TypeGenerator::<Rust>::absolute_target_path(&component, &config).unwrap();
        println!("target path: {target_path:?}");
        let filename = format!("{}.rs", component.type_name().to_case(Snake));
        assert!(target_path.ends_with(filename));
        let code_gen_result = TypeGenerator::<Rust>::generate_type(&component, &config, &resolver);
    ...
    #[test]
    pub fn test_type_generator_for_language_generate_component_rust_trait() {
        let component = Component::random_type().unwrap();
        let resolver = TypeResolver::new();
        ...
        let filename = format!("{}.rs", component.type_name().to_case(Snake));
        assert!(target_path.ends_with(filename));
        let code_gen_result = component.generate_type_for_target(CodeGenerationTargets::Rust, &config, &resolver);
        println!("{code_gen_result:?}");
        let code_gen_result = code_gen_result.unwrap();
        println!("resulting path: {:?}", code_gen_result.path);
        assert_eq!(target_path, code_gen_result.path);
