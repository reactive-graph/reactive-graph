...
impl TypeGenerator<Java> for Component {
...
mod tests {
    ...
    #[test]
    pub fn test_generate_component_java_interface() {
        ...
        let target_path = TypeGenerator::<Java>::absolute_target_path(&component, &config).unwrap();
        println!("target path: {target_path:?}");
        let filename = format!("{}.java", component.type_name().to_case(UpperCamel));
        assert!(target_path.ends_with(filename));
        let code_gen_result = TypeGenerator::<Java>::generate_type(&component, &config, &resolver);
