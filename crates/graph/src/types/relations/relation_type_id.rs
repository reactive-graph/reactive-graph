...
impl AsRef<NamespacedType> for RelationTypeId {
    fn as_ref(&self) -> &NamespacedType {
        &self.0
    }
}

impl AsRef<Namespace> for RelationTypeId {
...
impl RelationTypeIds {
    ...
    pub fn relation_type<TypeId: Into<RelationTypeId>>(self, ty: TypeId) -> Self {
        self.ty(ty)
    }
...
impl JsonSchema for RelationTypeIds {
    fn schema_name() -> Cow<'static, str> {
        "RelationTypeIds".into()
    }

    fn json_schema(schema_generator: &mut SchemaGenerator) -> Schema {
        let sub_schema: Schema = schema_generator.subschema_for::<RelationTypeId>();
        json_schema!({
            "type": "array",
            "items": sub_schema,
        })
...
impl FromIterator<Self> for RelationTypeIds {
...
mod tests {
    ...
    fn relation_type_id_test() {
        ...
        let ty = RelationTypeId::from_str(fully_qualified_namespace.as_str()).expect("Failed to parse entity type id from str");

        // Test parse from namespace
        let ty2 = RelationTypeId::from_str(&fully_qualified_namespace).expect("Failed to parse entity type id from string");
        assert_eq!(ty, ty2);

        // Test parse from optional namespace
        let ty3 = RelationTypeId::parse_optional_namespace(Some(fully_qualified_namespace.clone())).expect("Failed to parse entity type id from string");
        assert_eq!(Some(ty.clone()), ty3);
        assert_eq!(None, RelationTypeId::parse_optional_namespace(None).unwrap());
    ...
    fn relation_type_id_new_from_namespaced_type_test() {
        ...
        let ty = RelationTypeId::new(nt.clone());
    ...
    fn relation_type_id_from_namespaced_type_test() {
        let nt = NamespacedType::random_type_id().unwrap();
        let ty = RelationTypeId::from(nt.clone());
        assert_eq!(nt.namespace(), ty.namespace());
        assert_eq!(nt.path(), ty.path());
        assert_eq!(nt.type_name(), ty.type_name());
    }
    ...
    fn relation_type_id_from_string_test() {
        let ty1 = RelationTypeId::from_str("namespace::Type").unwrap();
        ...
        assert!(RelationTypeId::from_str("namespace::namespace::Type").is_ok());
        assert!(RelationTypeId::from_str("namespace::namespace::namespace::Type").is_ok());
        assert!(RelationTypeId::from_str("namespace::namespace::namespace::namespace::Type").is_ok());

        assert!(RelationTypeId::from_str("Namespace::Type").is_err());
        assert!(RelationTypeId::from_str("namespace::type").is_err());
        assert!(RelationTypeId::from_str("namespace::Namespace::Type").is_err());
        assert!(RelationTypeId::from_str("namespace::namespace::type").is_err());
        assert!(RelationTypeId::from_str("Namespace::namespace::type").is_err());
        assert!(RelationTypeId::from_str("namespace::Namespace::type").is_err());
        assert!(RelationTypeId::from_str("::Namespace::Type").is_err());
    ...
    #[test]
    fn relation_types_test() {
        let ty1 = RelationTypeId::from_str("namespace::Type1").unwrap();
        let ty2 = RelationTypeId::from_str("namespace::Type2").unwrap();
        let tys1 = RelationTypeIds::new().relation_type(&ty1).relation_type(&ty2);
        let tys2 = RelationTypeIds::new().relation_type(&ty2).relation_type(&ty1);
        assert_eq!(tys1, tys2);
    }
