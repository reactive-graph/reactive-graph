...
use dashmap::DashMap;
use dashmap::DashSet;
use inexor_rgf_reactive_service_api::EventChannels;
...
// pub struct SystemEventChannels(DashMap<u128, (Sender<Value>, Receiver<Value>)>);
//
...
impl ReactiveRelationManager for ReactiveRelationManagerImpl {
    ...
    fn register_reactive_instance(&self, reactive_relation: ReactiveRelation) -> Result<ReactiveRelation, ReactiveRelationRegistrationError> {
        ...
        // {
        //     Ok(_edge_key) => {
        //         self.reactive_relation_instances.insert(edge_key.clone(), reactive_relation_instance.clone());
        //         // Apply all components that are predefined in the relation type
