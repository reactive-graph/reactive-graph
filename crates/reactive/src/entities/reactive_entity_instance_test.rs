...
fn reactive_entity_instance_test() {
    ...
    properties.insert(
        property_name.clone(),
        ReactiveProperty::new(Uuid::new_v4(), property_name.clone(), Mutable, property_value.clone()),
    );
    ...
    let reactive_entity_instance = Arc::new(ReactiveEntity {
        ty: ty.clone(),
        id: uuid.clone(),
...
fn reactive_entity_instance_from_vertex_properties_test() {
    ...
    let reactive_entity_instance = Arc::new(ReactiveEntity::try_from(vertex_properties).unwrap());
    assert_eq!(type_name.clone(), reactive_entity_instance.type_name());
    assert_eq!(uuid.clone(), reactive_entity_instance.id.clone());
...
fn create_reactive_entity_instance_benchmark(bencher: &mut Bencher) -> impl Termination {
    ...
    bencher.iter(move || {
        ...
        let _reactive_entity_instance = Arc::new(ReactiveEntity {
            ty: ty.clone(),
            id: uuid.clone(),
            description: description.clone(),
            properties,
