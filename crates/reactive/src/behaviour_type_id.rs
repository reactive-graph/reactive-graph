...
impl BehaviourTypeIds {
    pub fn new() -> Self {
        BehaviourTypeIds(DashSet::new())
    }

    pub fn to_vec(&self) -> Vec<BehaviourTypeId> {
        let mut tys: Vec<BehaviourTypeId> = self.0.iter().map(|ty| ty.clone()).collect();
        tys.sort();
        tys
    }
}
...
impl From<Vec<BehaviourTypeId>> for BehaviourTypeIds {
    fn from(tys: Vec<BehaviourTypeId>) -> Self {
        BehaviourTypeIds(tys.into_iter().collect())
    }
}
...
impl From<DashSet<BehaviourTypeId>> for BehaviourTypeIds {
    fn from(tys: DashSet<BehaviourTypeId>) -> Self {
        BehaviourTypeIds(tys)
    }
}

impl From<&DashSet<BehaviourTypeId>> for BehaviourTypeIds {
    fn from(tys: &DashSet<BehaviourTypeId>) -> Self {
        BehaviourTypeIds(tys.clone())
    }
}
...
mod tests {
    ...
    impl DefaultTest for BehaviourTypeId {
        fn default_test() -> Self {
            BehaviourTypeId::new_from_type(r_string(), r_string())
        }
    }
